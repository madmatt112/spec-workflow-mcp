...
| `spawn.end` | plugin hook (`SubagentStop`) for every `sdd-*` agent; the launcher for a DeepSeek-provider worker | `agent`, `provider`, `input`, `output`, `cacheWrite`, `cacheRead`, `tokens` (digits or `unknown`), `cacheWrite5m`, `cacheWrite1h` (5-minute and 1-hour cache writes; digits or `unknown`), `gapRewrites` (calls more than 300 s after the previous call that wrote more than half its prefix; digits or `unknown`), `model`, `agentId` (hook rows). The hook writes a new row at each `SubagentStop` whose usage changed, so a yielding orchestrator has several; the latest row per `agentId` is the spawn's total |

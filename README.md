...
- **SDD Harness** — a Claude Code plugin (`spec-workflow-harness`, also bundled into the two MCP plugins) that runs the whole workflow without a human at the dashboard: say "continue the sdd process" and a supervisor takes the active spec through requirements, design, tasks, implementation (with a PR, gated on its CI checks), a retrospective conversation and a close-out that implements the approved retrospective plan, using pinned orchestrator and worker agents. The agent approves its own documents through the `approvals` tool's `approve` and `prune` actions. `spec-workflow-mcp --watch <spec store>` shows the run live in a terminal. See [docs/SDD-HARNESS.md](docs/SDD-HARNESS.md).

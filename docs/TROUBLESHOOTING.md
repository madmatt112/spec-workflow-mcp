...
npx -y @madmatt112org/spec-workflow-mcp@latest --help
...
**Error**: `npm ERR! 404 Not Found - @madmatt112org/spec-workflow-mcp@latest`
...
3. Try without version tag:
   ```bash
   npx @madmatt112org/spec-workflow-mcp /path/to/project
   ```
4. Install globally first:
   ```bash
   npm install -g @madmatt112org/spec-workflow-mcp
...
4. Try with absolute path:
   ```bash
   npx -y @madmatt112org/spec-workflow-mcp@latest $(pwd)
   ```
...
1. **Claude Desktop**: Check configuration file:
   ```json
   {
     "mcpServers": {
       "spec-workflow": {
         "command": "npx",
         "args": ["-y", "@madmatt112org/spec-workflow-mcp@latest", "/absolute/path/to/project"]
       }
     }
   }
   ```
...
2. **Claude Code CLI**: Verify setup:
   ...
   claude mcp add spec-workflow npx @madmatt112org/spec-workflow-mcp@latest -- /path/to/project
   ```
...
1. Verify dashboard is started:
   ```bash
   npx -y @madmatt112org/spec-workflow-mcp@latest /path --dashboard
   ```
...
1. Use a different port:
   ```bash
   npx -y @madmatt112org/spec-workflow-mcp@latest /path --dashboard --port 3456
   ```
...
1. Ensure dashboard is running alongside MCP server:
   ...
   npx -y @madmatt112org/spec-workflow-mcp@latest /path
   ```
...
4. Try reinstalling extension (build from source):
   ```bash
   code --uninstall-extension madmatt112.spec-workflow-mcp
...
1. Check argument order:
   ```bash
   # Correct
   npx -y @madmatt112org/spec-workflow-mcp@latest /path --dashboard --port 3000

   # Wrong
   npx -y @madmatt112org/spec-workflow-mcp@latest --dashboard /path --port 3000
   ```
...
2. Use local installation:
   ```bash
   npm install @madmatt112org/spec-workflow-mcp
   node node_modules/@madmatt112org/spec-workflow-mcp/dist/index.js /path
   ```
...
```bash
# Use forward slashes
npx -y @madmatt112org/spec-workflow-mcp@latest C:/Users/name/project

# Or escaped backslashes
npx -y @madmatt112org/spec-workflow-mcp@latest "C:\\Users\\name\\project"
```
...
1. **GitHub Issues**: [Create an issue](https://github.com/madmatt112/spec-workflow-mcp/issues)
...
npx -y @madmatt112org/spec-workflow-mcp@latest /path --debug
```

  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        disabled={isUpdating}
        className={`px-2 sm:px-3 py-1 text-xs rounded-md whitespace-nowrap transition-colors flex items-center gap-1.5 border border-[var(--border-default)] ${config.bgColor} ${config.textColor} ${config.hoverBg} ${
      ...
      {isOpen && !isUpdating && (
        <div className="absolute top-full mt-1 left-0 bg-[var(--surface-panel)] border border-[var(--border-default)] rounded-md shadow-lg z-50 min-w-[120px]">
          ...
          <div className="grid grid-cols-1 gap-3 sm:gap-4">
            {filteredAndSortedTasks?.map((task: any) => (
              <div
                ...
                className={`bg-[var(--surface-panel)] border border-[var(--border-default)] rounded-md p-4 sm:p-6 transition-all hover:shadow-md`}
              >
                          ...
                          <button
                            ...
                            className={`px-2 sm:px-3 py-1 sm:py-1.5 text-xs rounded-md transition-colors flex items-center gap-1 min-h-[32px] sm:min-h-[36px] border ${
                              copiedTaskId === task.id
                                ...
                                : 'bg-[var(--surface-base)] text-[var(--text-secondary)] border-[var(--border-default)] hover:bg-[var(--surface-raised)]'
                            }`}

...
- **Interfaces:**
  - `cacheTtlOf(raw: string | undefined): string` — returns the trimmed capture of `/cacheTtl:([^}]*)\}/` on the raw value the per-line split stores under the frontmatter key experimental (`scripts/sync-plugin-assets.cjs:94-98`), or the string "default" when the value is absent, has no match, or the capture trims to empty.
...
- **Interfaces:**
  ...
  - `agentLines` (`src/watch/render.ts:192`): the `declared` text (`src/watch/render.ts:220`) becomes `model effort` plus the profile's declared cache lifetime when `profile.cacheTtl` is set and is not "default". The pad at line 226 becomes `Math.max(24, stripAnsi(declared).length + 1)`.
...
- **Interfaces:**
  - `UsageCell` (`src/watch/usage.ts:13`) becomes `{ spawns, tokens, unknown, cacheWrite5m, cacheWrite1h, gapRewrites, cacheUnknownWrite, cacheUnknownGap }`, all numbers; `emptyCell` (`src/watch/usage.ts:57-59`) and `addCell` (`src/watch/usage.ts:65-69`) cover all eight.
...
- **Interfaces:** four files under `/tmp/scratchpad/sdd/agent-cache-ttl/`, written by an implementation task:
  - `e2e-setup.sh <checkout>` sets `E2E=/tmp/scratchpad/sdd/agent-cache-ttl/e2e`, `H=$E2E/claude-home` and `ROOT=$E2E/root`. It runs `npm run build` and `node scripts/sync-plugin-assets.cjs` in `<checkout>`. It runs `CLAUDE_CONFIG_DIR=$H bash <checkout>/scripts/dev-link.sh`, which links the branch's agents and skills into `$H` and registers the branch's hook in `$H/settings.json` (`scripts/dev-link.sh:19`, `scripts/dev-link.sh:37-38`, `scripts/dev-link.sh:50-64`). It writes the probe agents `$H/agents/sdd-cache-probe.md` and `$H/agents/sdd-cache-probe-worker.md`. It runs `git init $ROOT` and writes `$ROOT/.mcp.json` in the shape of `./.mcp.json:1-12`, with the args `<checkout>/dist/index.js`. It writes `$ROOT/.spec-workflow/spec-decomposition/decomposition.md` with one entry, `cache-probe` (a one-function greeting). It creates `$ROOT/.spec-workflow/specs/cache-gap-probe/`. It writes three UUIDs to `$E2E/sid-1`, `$E2E/sid-2` and `$E2E/sid-5`, then prints the launch commands below.
  ...
  - `sdd-cache-probe.md`: `name: sdd-cache-probe`. It has the `model:`, `effort:` and `experimental:` lines copied by `grep -E '^(model|effort|experimental):'` from `<checkout>/harness/agents/sdd-document-orchestrator.md`, and `tools: [Agent]`. Its body: spawn `sdd-cache-probe-worker` in the foreground with the prompt `wait`, then reply `DONE`.
  ...
  - `recompute.mjs` does not import or copy the hook code (Requirement 6 criterion 3). It reads `$ROOT/.spec-workflow/specs/{cache-probe,cache-gap-probe}/harness-events.jsonl` and the `harness-activity.jsonl` beside each (`harness/hooks/sdd-activity.sh:28-29`). It joins each `spawn.end` row to the `agent.stop` activity line with the same `ts` and `agent`; the hook writes both from one `ts` (`harness/hooks/sdd-activity.sh:96`, `harness/hooks/sdd-activity.sh:140`, `harness/hooks/sdd-activity.sh:157`). It opens `$H/projects/*/SESSION/subagents/agent-AGENTID.jsonl`, recomputes the three values from its own reading of Requirement 3 criteria 2 to 7, and prints one match or mismatch line per row. It prints the four evidence lines of the data model below. With `--write PATH`, it replaces the four scenario lines in that file.
- **Launch sequence** (operator, after the PR opens and before the merge):
  1. `bash /tmp/scratchpad/sdd/agent-cache-ttl/e2e-setup.sh <checkout>`
...
- **v1** (2026-09-24) — Initial draft.
  - **Lint pass.** 18 fixed; rejected: none.
    - L-1: cited `harness/hooks/hooks.json:27-37` for `SubagentStop`.
    - L-2, L-3: unbackticked `experimental`/`default` in the `cacheTtlOf` bullet, renamed its parameter to `raw`.
    - L-4: repointed the `agentLines` name citation to `src/watch/render.ts:192` (the function definition).
    - L-5, L-6: reworded the tier-line text to drop the bare `cacheTtl` placeholder and the bare `default` token.
    - L-7, L-8: gave `emptyCell` and `addCell` full-path citations (`src/watch/usage.ts:57-59`, `:65-69`).
    - L-9: replaced the bare `WORKTREE` placeholder with `<checkout>` throughout C8 (also in the two other bullets using it).
    - L-10: cited `.mcp.json` as `./.mcp.json:1-12`.
    - L-11, L-12: gave the activity/spawn.end line-number mentions full-path citations (`harness/hooks/sdd-activity.sh:28-29`, `:140`, `:157`).
    - L-13 through L-18: split the `cache-ttl-probe.test.ts` bullet so the env-var names sit on a line with no citation to resolve against.

...
9. WHEN run on the `question-gates` ledger THEN the spec total SHALL equal 1,963,320 plus nothing (the sum of its 23 digit-string rows), with 44 spawns and `unknown` marks on the cells of its 14 `unknown` and `na` rows; the seven orchestrator spawns, whose rows carry no `tokens` key, add to the spawn count and carry no mark, and the ledger has no `0` row.
...
- D8 — An unknown spawn (no digit-string `tokens` on any row, and a row that states `unknown` or `na`) shows as `<sum> (+n unknown)` in its cell, and a spawn with no `tokens` key on any row counts without a mark: options were print `unknown` for the whole cell or drop the spawn; chosen because the spawn count stays honest and the known sum stays usable.
...
- **v5** (2026-09-19) — Post-cap corrective pass, adjudicated, not re-reviewed.
  - **R4-1 — fixed (MUST_FIX).** Requirement 5 criterion 4 now defines the unknown mark on one boundary: a spawn with no digit-string token counts one spawn and no tokens, and is marked only when one of its rows states unknown or na; a spawn whose rows carry no token key at all counts with no mark; a zero is a digit string and a known zero, summed and never marked. Criterion 9 drops the vacuous zero from its 14 mandated marks and pins 44 spawns with the seven unmarked orchestrator spawns; D8 restated to match. Replayed over the fixture: 44 spawns, 1,963,320 tokens, 14 marks, 7 unmarked.
  - **R4-2 — fixed (SHOULD_FIX).** Requirement 5 criterion 5 defines the header run count as the number of distinct run values over every row the report reads, not the number of run-start rows, so the fixture header reads 2.

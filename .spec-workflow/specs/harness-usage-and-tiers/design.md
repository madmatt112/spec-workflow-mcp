...
- **Interfaces:**
  ...
  - `tokensTotal` (`src/watch/ledger.ts:316`) unchanged (Req 4.4); `src/watch/ledger.ts:308`'s `agent.stop` fill (join `:305-311`) gains `s.endedAt !== undefined`.
...
- **Interfaces:** each edit replaces the cited span:
  - Drop the `tokens=<n>` clause from the `spawn.usage` key lists at `harness/skills/sdd-document-phase/SKILL.md:46-48`, `harness/skills/sdd-implementation-phase/SKILL.md:57-58` and `harness/skills/sdd-closeout-phase/SKILL.md:48-49`; `harness/skills/sdd-document-phase/SKILL.md:114-116` drops "and tokens"; `:144` reads "from its report".
  - `harness/skills/sdd-retrospective/SKILL.md:35-39`: retains `phase.start phase=retrospective` (line 35) and `phase.end phase=retrospective result=retro-ready` (lines 39-40); only the clause between changes: `spawn.start` before and `spawn.usage` (`agent=sdd-retro-analyst role=proposals result=<one line>`) after the analyst; the hook writes the analyst's `spawn.end` (Req 2.4).
  ...
  - `docs/SDD-HARNESS.md:284-298`: a supervisor row (main session, no agent file, outside the generated profiles), four rows matching Req 6.1's groups with full ids, and one sentence naming `harness/agent-profiles.json` as generated by `scripts/sync-plugin-assets.cjs`, checked in CI; `:325-328`: the `SubagentStop` hook measures tokens from the transcript, the watch header sums them, `harness usage` splits them by kind.
...
4. **`agent-profiles.json` missing or malformed at run time:** `loadAgentProfiles` returns `{}`; entries render without a tier line; `--watch` and the server start (Req 3.5).
...
6. **`usage` on a spec directory that does not exist:** `{ success: false, message: "Failed to read <path>: ..." }` as `phase-log` (`src/tools/harness.ts:664-670`), for `compareSpecName`; a `safeJoin` throw (`src/core/path-utils.ts:190-194`) is returned the same way.
...
- **End-to-end** (the entry's scenario, run by the verifier from the worktree with `HOOK=<worktree>/harness/hooks/sdd-activity.sh`):
  ...
  2. The same with `agent_type: sdd-document-orchestrator` and `agent-a8a51b659b21c6e12.jsonl`: a `spawn.end` row appears (scenario 2). The live half (a real session's `SubagentStop`) runs after merge and restart (the registered hook is the main checkout's, `scripts/dev-link.sh:21`); record a deferral tagged `verification` (D13).
...
- D4 — `PHASE_ORDER` moves to `ledger.ts`, exported: options were export from `render.ts` or duplicate in `harness.ts`; chosen because the order is a model fact and `render.ts` imports `ledger.ts`.
...
- D9 — `agent.stop` carries numeric `tokens` when the transcript parsed: options were leave it or add it; chosen because `ActivityEvent.tokens` and the ticker (`src/watch/ledger.ts:357`) exist.
...
- D11 — `usage` treats a missing ledger as an empty report: options were fail naming the path or empty; chosen because `phase-log` treats `ENOENT` identically (`src/tools/harness.ts:673-683`).
...
- D13 — Scenarios (1) and (2) run the worktree's script by hand on real transcripts; the live-hook half is deferred past merge: options were skip both until merge, or split; chosen because the registered hook is the main checkout's and the deterministic half proves the sums.
- D14 — Generation lives inside `sync-plugin-assets.cjs`: options were a new script wired into `build`, or the existing one; chosen because Req 3.1 names the script and `check:plugin-assets` gates CI.
...
- The supervisor's tokens are not measured (no `SubagentStop` for the main session).
...
- **v2** (2026-09-19) — Round-1 adversarial response (adversarial-analysis-design.md, verdict iterate 1/1/2).
  ...
  - **Lint pass.** 1 fixed (L-2): the root-level vitest.config.ts citation gained the leading dot-slash the checker requires for a path with no directory; verified the fixture-exclusion text cited is still there. Rejected (77): L-1, L-3 through L-78 — rule 11, the same citation-identifier tokens the v1 lint pass already rejected with reasons on record, re-fired only because v2 shifted their line numbers; spot-verified against the real tree — buildModel is defined in ledger.ts outside its cited range; loadAgentProfiles is not inside the render.ts import line it cites; main lives elsewhere in sync-plugin-assets.cjs than its cited range; readUsage, usage and transcript_path are new text this design introduces — each still new or proposed text, or attributed to a different citation in the same block, none literally inside the newly-flagged range.
- **v3** (2026-09-19) — Round-2 adversarial response (adversarial-analysis-design-r2.md, verdict iterate 0/2/0), SHOULD_FIX-only corrective pass.
  - **R2-1 — Accepted (SHOULD_FIX).** The retrospective skill's edit span opened on the phase-start instruction and closed partway through the phase-end instruction, so a literal replacement over the whole span would have dropped both ledger boundaries. Narrowed the edit's description so it states plainly that it retains the phase-start line and the phase-end line, and confines the replaced text to the spawn-start, spawn-usage and spawn-end clause sitting between them. Verified against the real skill file: the phase-start clause opens line 35 and the phase-end clause runs from the end of line 39 into line 40. Now reads at line 111.
  - **R2-2 — Accepted (SHOULD_FIX).** The "never both" head-line property was proven only for the spawn-usage fold; this design's own decision to have agent-stop carry tokens lets that value reach a still-open node through the activity join, so an interleaved double spawn-start could render a badge and a token count together, overflow the 80-column head line, and transiently double-count the run total. Pinned the mechanism: the join's agent-stop fill now requires the node to have already closed before it fills tokens, so a running node can never carry them. Verified against the real ledger module: the guarded branch sits at line 308, inside the join spanning lines 305 to 311. Now reads at lines 78 and 85.
  - **Lint pass.** 4 fixed (L-E1 through L-E4); rejected: the four citation-identifier warnings that only fired alongside them (loadAgentProfiles, main, readUsage, transcript_path, now cleared by the same rewrite), and the standing recurring set of 78 citation-identifier warnings (rule 11, unchanged tokens already rejected with reasons of record in the v1 and v2 lint passes), including the newly flagged endedAt at line 85, spot-checked against the real ledger module and confirmed to be new mechanism text this design introduces rather than an existing-code claim. The v2 lint-pass bullet's spot-check examples carried backticked file-and-line citations that the checker parses as real citations lacking a directory prefix; rewrote that bullet's examples as plain prose naming files but no line numbers, preserving the same meaning. Checked the v1 and v3 history bullets for the same backticked file-and-line construct; found none.

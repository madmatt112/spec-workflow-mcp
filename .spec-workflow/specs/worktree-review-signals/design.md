...
- **Interfaces:** inserted after resolution (`:440-457`), before the settled block (`:467-477`):
  ...
  Precedence in `computeDiffMethodologyState`: `rejection` first, then `noReviewableFiles`, then `diff === ''`, then truncation. `nextSteps` (`src/tools/review-task.ts:512-520`) and `projectContext` (`src/tools/review-task.ts:521-526`) are unchanged.
...
10. **Store write throws:** a write/rename failure inside `fn` re-throws from `withRegistryLock`'s `finally` (`src/core/registry-lock.ts:385-390`); `recordBase`/`recordAttribution` reject; Components 6 and 7 already await in try/catch; the response is unaffected. The orphaned temp/`.stale` file is never cleaned up, but is inert once gitignored (Data Models).
...
- **v3** (2026-09-18) — Round-2 adversarial response (adversarial-analysis-design-r2.md, verdict iterate 0/2/2), SHOULD_FIX-only corrective pass.
  ...
  - **Lint pass.** 3 fixed (L-13, L-14 — the `nextSteps`/`projectContext` bare ranges at line 107 had inherited `src/types.ts` from the prior citation in the same passage and read out of bounds; both ranges hold those fields in `src/tools/review-task.ts`, confirmed by reading the lines, so both citations now name that file explicitly; L-27 — the bare `registry-lock.ts:385-390` at line 241 now reads `src/core/registry-lock.ts:385-390`, its real path, confirmed by reading the cited lines); rejected: L-1 to L-4, L-15 to L-28b (each identifier already carries its own correct citation elsewhere in the same passage, is this design's own new vocabulary, or the citation supports a ruling rather than identifier presence, as in prior lint passes), L-5 to L-12 (the linter's identifier check bleeds across the run-on bullet at line 106; each name is either this design's own vocabulary or already cited elsewhere in the same passage); 73 `citation-bare` info findings not enumerated (traceability nits, cap has no slack).

# Reviser brief — question-gates design v2
...
from the findings below, then report in 150 words or fewer: files touched; each finding
as `<id>: accepted | partially accepted | rejected`; citations verified (count); the
...
- Findings:
  `/home/mcf/repo/spec-workflow-mcp/.spec-workflow/specs/question-gates/reviews/adversarial-analysis-design.md`.
- Memory:
...
4. Write v2 in place. Add the Revision History line
   `- **v2** (2026-09-16) — Round-1 adversarial response (adversarial-analysis-design.md,
   verdict iterate 0/4/2).` followed by one nested bullet per finding:
   `- **<id> — <Accepted | Partially accepted | Rejected> (<severity>).** <what changed,
   or why not>`. If the document carries a `Document version:` header, set it to v2.
...
9. After you accept a finding, search the document for every other place with the same
   ...
   finding.
10. A finding marked `Compounds: R0-<n>` lands in text a previous delta wrote: do not
    reword the clause again. Write one plain sentence of what the clause must claim,
    delete the old text, and probe the new claim as round 1 would. A claim you cannot
    probe is deleted, not kept.

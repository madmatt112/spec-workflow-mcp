# Reviser brief — question-gates requirements v2
...
- Document:
  ...
  removes one.
- Findings:
  `/home/mcf/repo/spec-workflow-mcp/.spec-workflow/specs/question-gates/reviews/adversarial-analysis-requirements.md`.
- Memory:
...
4. Write v2 in place. Add the Revision History line
   `- **v2** (2026-09-16) — Round-1 adversarial response (adversarial-analysis-requirements.md,
   verdict iterate 1/4/4).` followed by one nested bullet per finding:
...
10. A finding marked `Compounds: R<n>` lands in text a previous delta wrote: do not

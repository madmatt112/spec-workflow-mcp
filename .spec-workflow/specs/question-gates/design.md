...
- **Reuses:** the drafter frontmatter gains the harness MCP tool in all three plugin-prefixed forms, mirroring the reviser's single-MCP grant (`harness/agents/sdd-reviser.md:14-16`); the drafter body (`harness/agents/sdd-drafter.md:16-26`) gains one requirements-phase-only step. Design and tasks phases write no gate-A payload. When the Lint step (Component 4) fixes a finding whose line falls inside the `## Decisions taken in this document` section (Component 4's trigger), the document orchestrator re-spawns `sdd-drafter` once more with a brief naming that section; the drafter re-reads the lint-corrected section fresh and re-extracts and re-ranks the full set of up to five decisions exactly as it did for v1 (Req 2 AC 1), then re-puts the complete `{ items: GateADecision[] }` via `harness gate put slot=a` — put overwrites the whole file (Component 2), so a single-triple put would drop the rest — before the orchestrator returns `PHASE: gate-a`.
...
- **Interfaces:**
  - **Gate A:** in Step 1, in the `requirements` phase and `MODE: normal` only, after the Lint step lands its fix (`harness/skills/sdd-document-phase/SKILL.md:69-120`) and before Step 2's first round, return `PHASE: gate-a`; the drafter's triples already sit on the surface (Component 3). **Reword trigger:** the Lint step already keeps `LINT.findings` (each with a 1-based `line`, `src/core/lint-types.ts:17-24`) and `LINT.open` on the task list without reading the document body (`harness/skills/sdd-document-phase/SKILL.md:102-105,117-119`); the orchestrator takes `LINT.findings` minus `LINT.open` (this pass's fixed findings) and, from `grep -n '^#' <document>` (a permitted structure read, `harness/skills/sdd-document-phase/SKILL.md:20-22`), the line range from the `## Decisions taken in this document` heading to the next heading; a fixed finding whose `line` falls in that range triggers Component 3's re-spawn before gate-a fires. A resume that orients to a later step (`Step 2`/`Step 3`/`Step R`) never re-emits it (Req 2 AC 3), because Step 0's nextStep is never `Step 1` on resume.
  - **Gate B:** classes (b) new external dependencies and (c) work beyond the approved requirements are surfaced through the tasks-phase reviewer and reviser, never read directly by the orchestrator. In every tasks-phase Step 2 round, the orchestrator's own `## This round` prompt addition — the same injection point Step 1 item 3 already uses for `RE-DECIDED` flags (`harness/skills/sdd-document-phase/SKILL.md:80-85,127-131`) — instructs `sdd-reviewer` to number any such task as a normal finding tagged `[gate-b:T<task id>]` or `[gate-c:T<task id>]` in its title, and instructs `sdd-reviser`'s brief to state, in that finding's Revision History bullet reasoning (`harness/skills/sdd-document-phase/references/briefs.md:204-206`), whether the task was removed (Accepted) or intentionally kept (Rejected) and why. Across rounds the orchestrator collects every `[gate-b:*]`/`[gate-c:*]` id from `grep -n -E '\[gate-(b|c):' <document>`'s Revision History lines (a permitted read, `harness/skills/sdd-document-phase/SKILL.md:20-22`) — the same tracking Step 3 item 4 already does for the standoff tally (`harness/skills/sdd-document-phase/SKILL.md:167-168`) — and keeps the ones marked Rejected as VetoCandidateItems (Data Models), dropping any marked Accepted. On the first `approved` (Step 6, `harness/skills/sdd-document-phase/SKILL.md:236-245`): call `harness gate class-a` for class (a), fold the kept VetoCandidateItems in as classes (b)/(c), order all three most-consequential-first into one list (D5), and `harness gate put slot=b` with payload = `{ tasks: [{id,title}], veto: VetoItem[] }`. In `MODE: revision` it writes no list (Req 5 AC 6).
...
- **Interfaces:**
  ...
  - **Gate A:** on `PHASE: gate-a` from the dispatch loop (`harness/skills/sdd-continue/SKILL.md:146-210`): `harness gate get slot=a`; write the receipt to `specs/<spec>/questions.md` (decisions, no `answer`) and commit best-effort before asking (Req 2 AC 7). In block, ask the triples with AskUserQuestion, at most five across at most two calls, each decision capped at four options (D9); a decision's reply is approve when its selected-option text matches `options[0]` verbatim with no appended free text, else needs revision (Req 2 AC 5); a denied/errored/timed-out second call keeps the first call's answers and treats the rest as `no answer` (Req 2 AC 4). Any needing-revision decision re-spawns the document orchestrator once with `MODE: revision` and `REVISION_INPUT` naming each decision's new option and free text; all-approve re-spawns `MODE: normal`. In `record`, write the decisions and `no answer`, a HANDOFF row, and re-spawn `MODE: normal` (Req 3). **Resume recheck:** at step 3 rule 4 (`harness/skills/sdd-continue/SKILL.md:122`), before dispatching phase `requirements`, the supervisor checks whether the gate-A receipt in `questions.md` is unanswered; if so it resolves gate A now — ask in block mode or fall to record — exactly as it would for a fresh `PHASE: gate-a`, instead of re-spawning `MODE: normal` straight to round 1 (Req 2 AC 7).
...
`VetoCandidateItem` (classes (b)/(c); tracked by the orchestrator from Revision History bullets, never computed server-side or read from the document body, Component 4): `{ taskId: string, class: 'b'|'c', reason: string }` — `taskId` and `class` parsed from a finding's `[gate-b:T<id>]`/`[gate-c:T<id>]` tag, `reason` from its Rejected bullet's one-line text. Folded into `VetoItem` one for one: `taskId`→`taskId`, `reason`→`summary`, `class`→`class`; ranked alongside the class (a) items by the same orchestrator judgment (D5).
...
- **End-to-end:** the decomposition entry's four scenarios, run by `npm test` and `claude plugin validate . --strict` (agent-rules.md checks): gate A interactive (one changed answer yields v2 before any round; unchanged answers in `questions.md`), gate A headless (proceeds to round 1 on v1), gate B interactive (sensitive-path item ranks above a kept `[gate-b]`-tagged new-dependency item above a kept `[gate-c]`-tagged out-of-scope item; one revision round, then the annotate path's re-approval proceeds straight to implementation with no second ask, Req 5 AC 6), gate B headless (writes the veto list and proceeds).
...
- **v3** (2026-09-16) — Round-2 adversarial response (adversarial-analysis-design-r2.md, verdict iterate 0/3/2); SHOULD_FIX-only corrective pass.
  - **R2-1 — Accepted (SHOULD_FIX).** Compounds R1-3. The R1-3 fix's "rewrites only that triple" was false: `gate put` overwrites the whole file (Component 2), so a single-triple `put` would drop the other decisions. Deleted that clause; Component 3 now has the re-spawned drafter re-extract and re-rank the full set of up to five decisions from the lint-corrected section (as it did for v1) and re-`put` the complete item list. No sibling occurrence found (grep confirmed only Component 3 stated this).
  - **R2-2 — Accepted (SHOULD_FIX).** Compounds R1-3. The re-spawn's trigger ("a lint fix that rewords a decision") named no signal the orchestrator may observe (`harness/skills/sdd-document-phase/SKILL.md:20-22` bars it from reading the Decisions section). Deleted that clause; Component 4's gate-A bullet now states a provable trigger built only from data the orchestrator already holds without a body read — LINT.findings minus `LINT.open` (both already on its task list from the Lint step, `harness/skills/sdd-document-phase/SKILL.md:102-105,117-119`), each finding's 1-based `line` (`src/core/lint-types.ts:17-24`), compared against the Decisions section's line range from a permitted `grep -n '^#'` structure read. Same construct also existed in Component 3's mirrored trigger clause; fixed there too as part of R2-1's edit.
  - **R2-3 — Accepted (SHOULD_FIX).** Novel. Gate-B classes (b)/(c) told the orchestrator to judge them "from `tasks.md`/`requirements.md`," violating `harness/skills/sdd-document-phase/SKILL.md:20-22` and contradicting Component 4's own Purpose line. Rewrote Component 4's gate-B bullet: the tasks-phase reviewer tags any such task as a normal finding (`[gate-b:T<id>]`/`[gate-c:T<id>]`) via the same round-prompt injection point Step 1 item 3 already uses for `RE-DECIDED` flags; the reviser's Revision History bullet reasoning states whether the task was removed or intentionally kept; the orchestrator collects kept (Rejected) tags by grepping Revision History lines — a permitted read, the same tracking Step 3 item 4 already does for the standoff tally — never the document body. Added `VetoCandidateItem` to Data Models and updated the class (a)-fold paragraph and the gate-B end-to-end scenario to match. The Purpose line needed no wording change: it is now true of the method.
  - **Lint pass.** 11 fixed; rejected: none.
- **v3 amended** (2026-09-16) — tasks R1-1: added Component 5's step 3 rule 4 resume recheck for an interrupted, unanswered gate-A receipt (Req 2 AC 7's second sentence), so a fresh supervisor re-asks or falls to record instead of dispatching `MODE: normal` straight to round 1.

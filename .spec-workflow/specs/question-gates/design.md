...
- **Interfaces:**
  - **Gate A:** in Step 1, in the `requirements` phase and `MODE: normal` only, after the Lint step lands its fix (`harness/skills/sdd-document-phase/SKILL.md:69-120`) and before Step 2's first round, return `PHASE: gate-a`; the drafter's triples already sit on the surface (Component 3). **Reword trigger:** the Lint step already keeps `LINT.findings` (each with a 1-based `line`, `src/core/lint-types.ts:17-24`) and `LINT.open` on the task list without reading the document body (`harness/skills/sdd-document-phase/SKILL.md:102-105,117-119`); the orchestrator takes `LINT.findings` minus `LINT.open` (this pass's fixed findings) and, from `grep -n '^#' <document>` (a permitted structure read, `SKILL.md:20-22`), the line range from the `## Decisions taken in this document` heading to the next heading; a fixed finding whose `line` falls in that range triggers Component 3's re-spawn before gate-a fires. A resume that orients to a later step (`Step 2`/`Step 3`/`Step R`) never re-emits it (Req 2 AC 3), because Step 0's nextStep is never `Step 1` on resume.
  - **Gate B:** classes (b) new external dependencies and (c) work beyond the approved requirements are surfaced through the tasks-phase reviewer and reviser, never read directly by the orchestrator. In every tasks-phase Step 2 round, the orchestrator's own `## This round` prompt addition — the same injection point Step 1 item 3 already uses for `RE-DECIDED` flags (`harness/skills/sdd-document-phase/SKILL.md:80-85,127-131`) — instructs `sdd-reviewer` to number any such task as a normal finding tagged `[gate-b:T<task id>]` or `[gate-c:T<task id>]` in its title, and instructs `sdd-reviser`'s brief to state, in that finding's Revision History bullet reasoning (`references/briefs.md:204-206`), whether the task was removed (Accepted) or intentionally kept (Rejected) and why. Across rounds the orchestrator collects every `[gate-b:*]`/`[gate-c:*]` id from `grep -n -E '\[gate-(b|c):' <document>`'s Revision History lines (a permitted read, `SKILL.md:20-22`) — the same tracking Step 3 item 4 already does for the standoff tally (`SKILL.md:167-168`) — and keeps the ones marked Rejected as `VetoCandidateItem`s (Data Models), dropping any marked Accepted. On the first `approved` (Step 6, `harness/skills/sdd-document-phase/SKILL.md:236-245`): call `harness gate class-a` for class (a), fold the kept `VetoCandidateItem`s in as classes (b)/(c), order all three most-consequential-first into one list (D5), and `harness gate put slot=b` with payload = `{ tasks: [{id,title}], veto: VetoItem[] }`. In `MODE: revision` it writes no list (Req 5 AC 6).

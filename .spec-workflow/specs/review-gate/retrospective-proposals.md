# Retrospective proposals — review-gate

Written 2026-09-14 by the retro analyst from retrospective.md.

First spec under harness 5.4.0. Verdict on each 5.4.0 change this spec touched: keep the
v4 cap (P9 tunes the budget under it), keep the Sonnet reviser (P14, P19), keep the lean
templates, keep one approval per phase (P21), keep the context file but let it be
corrected (P17), tune the convergence rule (P12). Nothing reverts.
...
This is the first retrospective in this store, so cross-spec evidence comes from the
earlier spec's HANDOFF notes and the harness's own record of past specs.

1. **Fix the class, not the instance.** Seen in earlier specs (the round-section
   template's line "Every MUST_FIX after round 1 in past specs was a claim error
   introduced by the previous delta") and in all three phases here (F10, F19). Not
   project-specific, so it graduates into the harness reviser brief as P19 rule 9, not
   into a steering document.

2. **Put the enumerating command in the task, not the count.** Seen in the earlier spec
   in this repo (HANDOFF "Things worth carrying forward": hand-written site lists short
   in seven places) and here as F16 (a header stating a count). Rule text for
   `agent-rules.md`, new section `## Documents`: "A task that edits a list, or text
   that states a count or a length, gives the command that finds every member (`grep -n
   …`) and says to update the count word."

3. **Watch list (one spec, will recur on every spec in this repo).** P1 (node 20 is the
   CI runtime) and P2 (`plugins/` copies are generated) are already written as
   `agent-rules.md` lines; the next retrospective confirms them. F9 and F12 recurred in
   all three document phases; they are harness rules and P9 and P12 answer them.

...
8. **Preflight (a) fails or the key is unset in the preflight task:** the implementer reports `ESCALATE:`; the orchestrator reverts the task, records `escalation`, reports `PHASE: escalate`; no later task runs until a human rules (Req 6 crit 6-7).

...
| `spawn.usage` | orchestrator, right after a worker's report | `agent`, `role` (precise, e.g. `review v3`, `implement task 13`, `verify task 13`, `fix ci e2e round 1`, `implement harness batch 1`), `result` (VERDICT / VERIFY / logged line), `tokens` (the count the Agent result states in its footer; the only source of per-spawn tokens, since hook payloads carry no usage), `phase`, `task` or `round` |

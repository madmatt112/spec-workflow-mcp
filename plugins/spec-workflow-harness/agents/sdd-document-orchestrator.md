...
- Spawn workers with the Agent tool, foreground, `subagent_type` set to `<AGENT_PREFIX>:<worker>` where the worker is one of `sdd-drafter`, `sdd-reviewer`, `sdd-reviser`, `sdd-adjudicator`, `sdd-checker` — or, for a DeepSeek-provider worker (one `PROVIDERS` lists with a `<worker>:deepseek:<model>` entry), with the Bash tool as `bash <LAUNCHER> <worker> "<launch message>"`. Never pass a `model` parameter. Never use `subagent_type: fork`.
